#ifndef SOLVE_OPENMP_AVX2_INCLUDED
#define SOLVE_OPENMP_AVX2_INCLUDED
...
namespace ConjugateGradient
{
	...
	private:
			...
			const std::size_t maxNonzero)
		{
			...
			const auto ONE = _mm256_set_epi64x(1, 1, 1, 1);
...
#pragma omp parallel for
			for(auto ii = Index(0); ii < static_cast<Index>(n - rest); ii += SIMD_COUNT)
			{
				...
				auto simdIdx = _mm256_setzero_si256();
				for(auto idx = decltype(maxNonzero)(0); idx < maxNonzero; idx++)
				{
					...
					simdIdx = _mm256_add_epi64(simdIdx, ONE);
				}
...
#pragma omp parallel for
	...
	public:
		...
		SolveOpenMPAvx2() {}
		SolveOpenMPAvx2(const SolveOpenMPAvx2&) = delete;
		SolveOpenMPAvx2(SolveOpenMPAvx2&&) = delete;
		SolveOpenMPAvx2& operator=(const SolveOpenMPAvx2&) = delete;
		SolveOpenMPAvx2& operator=(SolveOpenMPAvx2&&) = delete;
