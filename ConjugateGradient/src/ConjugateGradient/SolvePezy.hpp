#ifndef SOLVE_PEZY_INCLUDED
#define SOLVE_PEZY_INCLUDED
...
#include "pzcl/pzcl.hpp"
#include "pzcl/ocl_wrapper.hpp"

namespace ConjugateGradient
{
	// PEZYで解く
	class SolvePezy final
	{
	private:
		static constexpr auto THREADS = std::size_t(16384);
		...
		void SpMV(cl::Buffer& y, const cl::Buffer& data, const cl::Buffer& column, const cl::Buffer& nonzero, const std::size_t maxNonzero, const cl::Buffer& x)
		{
			...
			spmv.setArg(5, x);
			spmv.setArg(6, n);
			queue.enqueueNDRangeKernel(spmv, cl::NullRange, cl::NDRange(THREADS), cl::NullRange);
		}
		...
		void Add(cl::Buffer& y, const double alpha, const cl::Buffer& x)
		{
			...
			addAlpha.setArg(2, x);
			addAlpha.setArg(3, n);
			queue.enqueueNDRangeKernel(addAlpha, cl::NullRange, cl::NDRange(THREADS), cl::NullRange);
		}
		// ベクトルの加算y = x + βy
		cl::Kernel addBeta;
		void Add(cl::Buffer& y, const cl::Buffer& x, const double beta)
		{
			...
			addBeta.setArg(3, n);
			queue.enqueueNDRangeKernel(addBeta, cl::NullRange, cl::NDRange(THREADS), cl::NullRange);
		}
		// ベクトルの引き算z = x - y
		cl::Kernel sub;
		void Sub(cl::Buffer& z, const cl::Buffer& x, const cl::Buffer& y)
		{
			...
			sub.setArg(2, y);
			sub.setArg(3, n);
			queue.enqueueNDRangeKernel(sub, cl::NullRange, cl::NDRange(THREADS), cl::NullRange);
		}
		// 総和
		cl::Kernel sum;
		auto Sum(cl::Buffer z)
		{
			...
			for(auto m = n; m > 1;)
			{
				...
				sum.setArg(2, m);
				queue.enqueueNDRangeKernel(sum, cl::NullRange, cl::NDRange(THREADS), cl::NullRange);
			}
		...
		auto Dot(const cl::Buffer& x, const cl::Buffer& y)
		{
			...
			mul.setArg(2, y);
			mul.setArg(3, n);
			queue.enqueueNDRangeKernel(mul, cl::NullRange, cl::NDRange(THREADS), cl::NullRange);
			return Sum(z);
		}
		// ベクトルの内積r = x・x
		cl::Kernel square;
		auto Dot(const cl::Buffer& x)
		{
			...
			square.setArg(1, x);
			square.setArg(2, n);
			queue.enqueueNDRangeKernel(square, cl::NullRange, cl::NDRange(THREADS), cl::NullRange);
			return Sum(z);
		}
		// ベクトルの複製y = x
	...
	public:
		...
		SolvePezy() : n(0)
		{
			...
			if(platforms.size() == 0)
			{
				throw std::runtime_error{"No PEZY platform"};
			}
			...
			if(devices.size() == 0)
			{
				throw std::runtime_error{"No PEZY device"};
			}
			...
			#include "kernel/kernel.pz.inc"
			;
			pzcl::Program::Binaries binaries;
			binaries.push_back(std::make_pair(bin, sizeof(bin)));
		...
		SolvePezy(const SolvePezy&) = delete;
		SolvePezy(SolvePezy&&) = delete;
		SolvePezy& operator=(const SolvePezy&) = delete;
		SolvePezy& operator=(SolvePezy&&) = delete;
