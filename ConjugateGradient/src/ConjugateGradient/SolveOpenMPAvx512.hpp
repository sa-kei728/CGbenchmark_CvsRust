#ifndef SOLVE_OPENMP_AVX512_INCLUDED
#define SOLVE_OPENMP_AVX512_INCLUDED
...
namespace ConjugateGradient
{
	...
	private:
		...
		static constexpr auto SIMD_COUNT = sizeof(__m512d) / sizeof(double);
			...
			const std::size_t maxNonzero)
		{
			...
			const auto ONE = _mm512_set_epi64(1, 1, 1, 1, 1, 1, 1, 1);
			const auto rest = n % SIMD_COUNT;

#pragma omp parallel for
			for(auto ii = Index(0); ii < static_cast<Index>(n - rest); ii += SIMD_COUNT)
			{
				const auto i = static_cast<decltype(n)>(ii);

				auto y_i = _mm512_setzero_pd();

				const auto nnz = _mm512_load_epi64(reinterpret_cast<const __m512i*>(nonzero() + i));
				auto simdIdx = _mm512_setzero_si512();
				for(auto idx = decltype(maxNonzero)(0); idx < maxNonzero; idx++)
				{
					const auto a_ij = _mm512_load_pd(data() + i * maxNonzero + idx * SIMD_COUNT);
					const auto j = _mm512_load_epi64(column() + i * maxNonzero + idx * SIMD_COUNT);

					const auto mask = _mm512_cmpgt_epi64_mask(nnz, simdIdx);
					const auto x_j = _mm512_mask_i64gather_pd(ZERO, mask, j, x(), sizeof(double));

					y_i = _mm512_fmadd_pd(a_ij, x_j, y_i);
					simdIdx = _mm512_add_epi64(simdIdx, ONE);
				}

				_mm512_store_pd(y() + i, y_i);
			}
		...
		{
			const auto n = y.N;
			const auto a = _mm512_broadcast_f64x4(_mm256_broadcast_sd(&alpha));
...
#pragma omp parallel for
			for(auto ii = Index(0); ii < static_cast<Index>(n - rest); ii += SIMD_COUNT)
			{
				...
				const auto x_i = _mm512_load_pd(x() + i);
				const auto y_i0 = _mm512_load_pd(y() + i);

				const auto y_i = _mm512_fmadd_pd(a, x_i, y_i0);
				_mm512_store_pd(y() + i, y_i);
			}
		...
		{
			const auto n = y.N;
			const auto b = _mm512_broadcast_f64x4(_mm256_broadcast_sd(&beta));
...
#pragma omp parallel for
			for(auto ii = Index(0); ii < static_cast<Index>(n - rest); ii += SIMD_COUNT)
			{
				...
				const auto y_i = _mm512_fmadd_pd(b, y_i0, x_i);
				_mm512_store_pd(y() + i, y_i);
			}
...
#pragma omp parallel for
			for(auto ii = Index(0); ii < static_cast<Index>(n - rest); ii += SIMD_COUNT)
			{
				...
				const auto x_i = _mm512_load_pd(x() + i);
				const auto y_i = _mm512_load_pd(y() + i);

				const auto z_i = _mm512_sub_pd(x_i, y_i);
				_mm512_store_pd(z() + i, z_i);
			}
...
#pragma omp parallel for
			for(auto ii = Index(0); ii < static_cast<Index>(n - rest); ii += SIMD_COUNT)
			{
				...
				sum = _mm512_fmadd_pd(x_i, y_i, sum);
			}

			alignas(512/8) double rr[SIMD_COUNT];
			_mm512_store_pd(rr, sum);
...
#pragma omp parallel for
			for(auto ii = Index(0); ii < static_cast<Index>(n - rest); ii += SIMD_COUNT)
			{
				...
				const auto x_i = _mm512_load_pd(x() + i);
				sum = _mm512_fmadd_pd(x_i, x_i, sum);
			}
...
#pragma omp parallel for
	...
	public:
		...
		SolveOpenMPAvx512() {}
		SolveOpenMPAvx512(const SolveOpenMPAvx512&) = delete;
		SolveOpenMPAvx512(SolveOpenMPAvx512&&) = delete;
		SolveOpenMPAvx512& operator=(const SolveOpenMPAvx512&) = delete;
		SolveOpenMPAvx512& operator=(SolveOpenMPAvx512&&) = delete;
		...
		void PreProcess(const Problem& problem)
		{
			...
			constexpr std::size_t ALIGN = sizeof(__m512d);
