#ifndef SOLVE_CUSPARSE_INCLUDED
#define SOLVE_CUSPARSE_INCLUDED
...
namespace ConjugateGradient
{
	...
	private:
		...
		{
			cusparse::Dhybmv(handle, CUSPARSE_OPERATION_NON_TRANSPOSE,
				descr, m, 1, xx, 0, y);
		}
#else
		void SpMV(double y[], const double data[], const int column[], const int rowOffset[], const double xx[], const std::size_t n)
		{
			...
			cusparse::Dcsrmv(handle, CUSPARSE_OPERATION_NON_TRANSPOSE, nn, nn, totalNonzero,
	...
	public:
		...
		SolveCusparse() {}
		SolveCusparse(const SolveCusparse&) = delete;
		SolveCusparse(SolveCusparse&&) = delete;
		SolveCusparse& operator=(const SolveCusparse&) = delete;
		SolveCusparse& operator=(SolveCusparse&&) = delete;
...
#ifdef CUSPARSE_ELL
			// ELL形式に変換
			cusparse::Dcsr2Hyb(handle, descr, static_cast<int>(n), static_cast<int>(n), Adata.get(), Acolumn.get(), ArowOffset.get(), mat, static_cast<int>(nnz), CUSPARSE_HYB_PARTITION_MAX);
#endif
		...
		void Solve(const Problem& problem)
		{
...
#endif
			SolveCudaMain::Sub(r.get(), b.get(), Ap.get(), n);
			...
			for(auto i = decltype(loop)(0); i < loop; i++)
			{
				...
				SolveCudaMain::Add(x.get(), alpha, p.get(), n);
				SolveCudaMain::Add(r.get(), -alpha, Ap.get(), n);
				...
				SolveCudaMain::Add(p.get(), r.get(), beta, n);
				rr = rrNew;
